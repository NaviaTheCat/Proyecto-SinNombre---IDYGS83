...
function Asistente({ onClose }) {
  const [mensaje, setMensaje] = useState('')
  const [chat, setChat] = useState([
    { tipo: 'usuario', texto: '¿Hola me ayudas con un trámite?' },
    { tipo: 'asistente', texto: 'Hola soy tu asistente, dime exactamente que trámite necesitas para ayudarte' }
  ])
    ...
    <div className='h-full flex flex-col'>
      ...
      <div className='flex-1 overflow-y-auto bg-gray-50 p-3 md:p-4 rounded-lg mb-3 md:mb-4 flex flex-col gap-2 md:gap-3'>
        {chat.map((msg, index) => (
          <div key={index} className={`flex ${msg.tipo === 'usuario' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-xs p-2 md:p-3 rounded-lg text-xs md:text-sm ${
              msg.tipo === 'usuario' 
      ...
      <div className='flex gap-2'>
        <input 
          ...
          className='flex-1 border-2 border-blue-500 rounded-lg p-2 text-xs md:text-sm focus:outline-none'
        />
        <button 
